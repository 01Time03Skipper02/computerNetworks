...
func sendEmail(mail Mail) (code int, message string, err error) {
	...
	auth := smtp.PlainAuth("", "sasha.shvech010302@gmail.com", "kwczdpbkvxeozdhz",
		"smtp.gmail.com")
	...
	if err := c.Mail("sasha.shvech010302@gmail.com"); err != nil {
		code, _ = strconv.Atoi(err.Error()[:3])
		return code, err.Error()[3:], err
	}
...
func saveInDB(code int, msg string, err error, request Mail) {
	...
	_, err = db.Exec("insert into iu9networkslabs.iu9Lab7Shvets (recepientaddr, subject, msgbody, recepientname, responsecode, respdescription)values (?, ?, ?, ?, ?,?) ", request.To, request.Subject, string(request.Body), request.To, code, msg)
	if err != nil {
		panic(err)
	}
}
